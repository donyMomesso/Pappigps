  ...
  return (
    <section className="py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="rounded-[36px] border border-[#E5E7EB] bg-white p-8 shadow-sm md:p-10">
          <div className="grid gap-10 lg:grid-cols-[0.88fr_1.12fr] lg:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                {logos.map((logo) => (
                  <span
                    key={logo}
                    className="rounded-full border border-[#E5E7EB] bg-[#F9FAFB] px-4 py-2 text-sm font-medium text-[#374151]"
                  >
                    {logo}
                  </span>
                ))}
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {proofCards.map((card) => (
                <div
                  key={card.title}
                  className="rounded-[28px] bg-[#111827] p-6 text-white shadow-[0_25px_50px_-35px_rgba(17,24,39,0.8)]"
                >
                  <p className="text-4xl font-semibold tracking-tight text-[#FDBA74]">
                    {card.metric}
                  </p>
